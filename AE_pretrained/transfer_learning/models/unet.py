...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        # Now, after all attributes are defined, load the pretrained encoder (if provided)
    ...
    def load_pretrained_encoder(self, pretrained_path: str):
        """
        Load pre-trained encoder weights from a checkpoint.
        
        Args:
            pretrained_path: Path to the pre-trained model checkpoint
        """
        # Load the checkpoint
        checkpoint = torch.load(pretrained_path, map_location='cpu')
        
        # Identify the encoder state dict
        # This handles both full model checkpoints and state dict only checkpoints
        if 'model_state_dict' in checkpoint:
            encoder_state_dict = {
                k.replace('encoder_stages.', ''): v 
                for k, v in checkpoint['model_state_dict'].items() 
                if k.startswith('encoder_stages.')
            }
        elif 'encoder_stages' in checkpoint:
            # If checkpoint was saved with a different method
            encoder_state_dict = checkpoint['encoder_stages']
        else:
            # Last resort: try loading entire state dict if architectures match
            encoder_state_dict = {
                k: v for k, v in checkpoint.items() 
                if k.startswith('encoder_stages.')
            }
        
        # Load the state dict, handling potential slight mismatches
        try:
            self.encoder_stages.load_state_dict(encoder_state_dict)
            print(f"Successfully loaded pre-trained encoder from {pretrained_path}")
        except Exception as e:
            print(f"Error loading pre-trained encoder: {e}")
            print("Attempting partial loading...")
            # Attempt partial loading
            model_state = self.encoder_stages.state_dict()
            for name, param in encoder_state_dict.items():
                if name in model_state and model_state[name].shape == param.shape:
                    model_state[name].copy_(param)
            print("Partial pre-trained encoder weights loaded.")
        
        # Always freeze encoder stages
        for param in self.encoder_stages.parameters():
            param.requires_grad = False
        print("Encoder stages have been frozen.")